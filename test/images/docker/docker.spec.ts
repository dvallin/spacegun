...
describe("DockerImageProvider", () => {
    ...
    describe("image", () => {
        ...
        it("retrieves images", () => {
            expect(provider.image("someImage")).resolves.toEqual(
                { name: "someImage", tag: "latest", url: "repo/someImage:latest@abcd" }
            )
            expect(axios.get).toHaveBeenCalledWith(
                "http://repo/v2/someImage/manifests/latest",
        ...
        it("retrieves images with tags", () => {
            expect(provider.image("someImage", "someTag")).resolves.toEqual(
                { name: "someImage", tag: "someTag", url: "repo/someImage:someTag@abcd" }
            )
            expect(axios.get).toHaveBeenCalledWith(
                "http://repo/v2/someImage/manifests/someTag",
                { headers: { accept: "application/vnd.docker.distribution.manifest.v2+json" } }
            )
        })
