...
import {
  GitConfigRepository,
  fromConfig as gitRepoFromConfig
} from "./config/git/GitConfigRepository";
...
export class Application {
  ...
  private async initialize(config: Config): Promise<void> {
    ...
    initCluster(
      KubernetesClusterRepository.fromConfig(config.kube, config.namespaces)
    );
    ...
    if (process.env.LAYER === Layers.Server) {
      ...
      if (gitRepo !== undefined) {
        ...
        this.crons.register("config-reload", config.git!.cron, () =>
